...
// 添加节点：将new_n插入到prev和next之间。
static inline void __list_add(struct list_head *new_n,
...
// 添加new_n节点：将new_n添加到head之后，是new_n称为head的后继节点。
static inline void list_add(struct list_head *new_n, struct list_head *head)
{
    __list_add(new_n, head, head->next);
}

// 添加new_n节点：将new_n添加到head之前，即将new_n添加到双链表的末尾。
